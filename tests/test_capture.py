...
def test_an_explicit_backend_pins_exactly_one(name: str, expected: int) -> None:
...
def test_a_device_label_prefers_the_path() -> None:
    ...
    assert info.label == "[2] Fisheye (/dev/video2)"
...
def test_a_device_label_falls_back_to_the_index() -> None:
    ...
    assert info.label == "[1] Camera 1 (index 1)"
...
def test_enumeration_reports_a_working_device(driver: FakeDriver) -> None:
    found = enumerate_devices(max_index=3)
...
def test_enumeration_skips_an_index_that_will_not_open(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert [device.index for device in enumerate_devices(max_index=3)] == [1]


def test_enumeration_skips_a_device_that_streams_nothing(monkeypatch: pytest.MonkeyPatch) -> None:
    """A ``/dev/video*`` node can be a metadata interface with no video on it."""
...
def test_enumeration_stops_at_the_first_working_backend(monkeypatch: pytest.MonkeyPatch) -> None:
    """Otherwise one camera is reported once per backend that can see it."""
    fake = FakeDriver(devices={0: Device()})
    monkeypatch.setattr(cv2, "VideoCapture", fake)
